...
interface SlotState {
    target: number      // cible kcal pour ce créneau
    consumed: number    // kcal consommées dans ce créneau
    remaining: number   // target - consumed (peut être négatif si dépassé)
    locked: boolean     // true si le créneau est terminé
}
...
export const useAppStore = create<AppState>((set, get) => ({
    ...
    setTodayMeals: (meals) => {
        ...
        // Recalculer les slots depuis les repas chargés
        const { profile } = get()
        if (!profile) return
        ...
        // Verrouiller les créneaux passés et redistribuer
            ...
            return {
                slots: {
                    ...
                    [slot]: {
                        ...
                        remaining: newRemaining, // peut être négatif si dépassé
                    }
                }
            }
        })
    ...
    // ─── Mise à jour slot après suppression ──────────────────
    ...
    // ─── Redistribution à la fin d'un créneau ────────────────
    redistributeAfterSlot: (finishedSlot) => {
        ...
        // Calories restantes sur toute la journée
        const remainingCalories = Math.max(0, calorieTarget - dailyCalories)

        // Total des % des créneaux restants
